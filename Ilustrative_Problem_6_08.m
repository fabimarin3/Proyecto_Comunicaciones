...
T=1;
W=1/(2*T);
n=-(N-1)/2:(N-1)/2; %Largo de las iteraciones

for i=1:length(n),
    g_T(i)=0;
    for m=-(N-1)/2:(N-1)/2,
        if (abs((4*m)/(N*T))<=W),
            g_T(i)=g_T(i)+sqrt((1/W)*cos((2*pi*m)/(N*T*W)))*exp(1i*2*pi*m*n(i)/N); %Funcion del filtro
        end;
        echo off;
    end;
end;
echo on;
n2=0:N-1;
g_T=real(g_T); %Vuelve real la funcion del filtro 
...
ConvImp_in_dB=20*log10(abs(G_TT)/max(abs(G_TT))); %Convolucion 
figure(3);
